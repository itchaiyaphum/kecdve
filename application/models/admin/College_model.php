...
class College_model extends BaseModel
{
    ...
    {
        parent::__construct();
        $this->table = $this->ci->factory_lib->getTable('College');
    }

    public function getPagination()
    {
        return $this->ci->helper_lib->getPagination(array(
            'base_url' => '/admin/college/',
    ...
    public function getItems($options = array())
    {
        ...
        $sql = "SELECT * FROM college WHERE {$where}";
    ...
    {
        $filter_search = $this->ci->input->get_post('college_filter_search');
        $filter_status = $this->ci->input->get_post('college_filter_status');
