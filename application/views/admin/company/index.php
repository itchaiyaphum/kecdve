		...
		<div class="tm-main uk-width-medium-3-4 uk-margin-top uk-margin-bottom">
			<div class="uk-clearfix">
    			...
    			<div class="uk-float-right">
    				<a href="<?php echo base_url('/admin/company/edit');?>" class="uk-button uk-button-success"><i class="uk-icon-plus"></i> เพิ่ม</a>
    			</div>
			</div>
			<hr/>
            <form action="<?php echo base_url('admin/company');?>" method="post" name="adminForm">
            	<table class="uk-table">
            		<tr>
            			<td width="100%">
            				Filter:
            				<input type="text" name="company_filter_search" id="search" value="<?php echo set_value('company_filter_search');?>" class="text_area" onchange="document.adminForm.submit();" />
            			...
            			<td nowrap="nowrap">
            				<?php echo $this->helper_lib->getStatusHtml('company_filter_status');?>
            			</td>
            		</tr>
            	</table>
            	...
            	<table class="uk-table" cellpadding="1">
            		...
            		if(count( $items )<=0){
            			...
            			for ($i=0, $n=count( $items ); $i < $n; $i++)
            			{
            				...
            				$link_edit = base_url('admin/company/edit/?id='.$row->id.'&per_page='.$per_page);
            				$link_restore = base_url('admin/company/restore/?id='.$row->id.'&per_page='.$per_page);
            				$link_trash = base_url('admin/company/trash/?id='.$row->id.'&per_page='.$per_page);
            				$link_delete = base_url('admin/company/delete/?id='.$row->id.'&per_page='.$per_page);
            				
            				$status_link = base_url('admin/company/unpublish/?id='.$row->id.'&per_page='.$per_page);
            				if($row->status==0){
            				    $status_link = base_url('admin/company/publish/?id='.$row->id.'&per_page='.$per_page);
            				}
            			?>
            			<tr class="<?php echo "row$k"; ?>">
            				...
            				<td>
            					...
            					<?php 
            					if($this->helper_lib->getFilterStatus('company_filter_search')=='trash'){
            					?>
