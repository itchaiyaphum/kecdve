		...
		<div class="tm-main uk-width-medium-3-4 uk-margin-top uk-margin-bottom">
			<div class="uk-clearfix">
				<div class="uk-float-left">
					<h1>จัดการข้อมูลภาคการศึกษา</h1>
				</div>
    			<div class="uk-float-right">
    				<a href="<?php echo base_url('/admin/semester/edit');?>" class="uk-button uk-button-success"><i class="uk-icon-plus"></i> เพิ่ม</a>
    			</div>
			</div>
			<hr/>
            <form action="<?php echo base_url('admin/semester');?>" method="post" name="adminForm">
            	...
            	<table class="uk-table" cellpadding="1">
            		...
            		if(count( $items )<=0){
            			...
            			for ($i=0, $n=count( $items ); $i < $n; $i++)
            			{
            				...
            				$link_edit = base_url('admin/semester/edit/?id='.$row->id.'&per_page='.$per_page);
            				$link_restore = base_url('admin/semester/restore/?id='.$row->id.'&per_page='.$per_page);
            				$link_trash = base_url('admin/semester/trash/?id='.$row->id.'&per_page='.$per_page);
            				$link_delete = base_url('admin/semester/delete/?id='.$row->id.'&per_page='.$per_page);
            				
            				$status_link = base_url('admin/semester/unpublish/?id='.$row->id.'&per_page='.$per_page);
            				if($row->status==0){
            				    $status_link = base_url('admin/semester/publish/?id='.$row->id.'&per_page='.$per_page);
            				}
            			?>
